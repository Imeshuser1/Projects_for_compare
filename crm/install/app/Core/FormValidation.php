...
{ 
    ...
    public function validate($data = [])
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($data['csrf_token'] != $_SESSION['csrf_token']) {
                ...
                $this->inputPost($data['hostname'],'Host Name',['required','hostname_regx','password']);
    ...
    public function inputPost($input = null, $title = null, $condition = null)
    {
        ...
        if (in_array('hostname_regx', $condition)) {
            if ($this->hostname_regx($input) === false) {
                $this->exception []="The {$title} entered is not a valid {$title}";
            }
        }
    ...
    public function hostname_regx($input = null)
    {
        if (preg_match("/^([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])(\.([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]{0,61}[a-zA-Z0-9]))*$/", $input)) {
            return true;
        } else {
            return false;
        }
