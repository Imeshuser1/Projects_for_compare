...
{  
    ...
    public function checkPhpVersion($required = null, $condition = null) {
        ...
        if (version_compare($phpVersion, $required, $condition)) {
            //unsuccess
            $this->versions['PHP'] =  "<i class='fa fa-times red'></i> You have <strong class='red'> PHP $phpVersion </strong> (Required <strong> $required</strong> or greater)";
            $this->error[] = true;
    ...
    public function checkMySqlVersion($required = null, $condition = null) {
        ...
        preg_match('/[1-9].[0-9].[0-9]+/', $mysql, $search);    
        if (version_compare($search[0],  $required, $condition)) {
            $this->versions['PDO MySql Client API'] =  "<i class='fa fa-times red'></i> You need <strong class='red'>PDO MySql Client API version $required</strong> or greater";
            $this->error[] = true;
            $this->version_error = true;
        } else {    
            $this->versions['PDO MySql Client API'] = "<i class='fa fa-ok green'></i> You have <strong class='green'>PDO MySql Client API $search[0]</strong> (Required <strong>$required</strong> or greater)"; 
        }         
    }
