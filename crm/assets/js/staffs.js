function Staffs_Controller($scope, $http, $mdSidenav, $mdDialog, $filter) {
	...
	$http.get(BASE_URL + 'api/table_columns/' + 'staff').then(function (Data) {
	...
	$scope.updateColumns = function(column, value) {
		...
		var posturl = BASE_URL + 'api/update_columns/staff';
	...
	$http.get(BASE_URL + 'staff/get_staff').then(function (Staff) {
		...
		$scope.AddStaff = function(){
			...
			//update create link//
			var posturl = BASE_URL + 'staff/create_staff/';
			$http.post(posturl, dataObj, config)
				.then(
					function (response) {
						$scope.saving = false;
...
function Staff_Controller($scope, $http, $mdSidenav, $mdDialog, $filter, fileUpload) {
		...
		$http.post(BASE_URL + 'staff/changestaffpassword_admin/'+ STAFFID, dataObj, config)
			.then(
				function (response) {
					...
					if (response.data.success == true) {
						globals.mdToast('success', response.data.message);
						$scope.apassword.newpassword = '';
						$scope.apassword.c_newpassword = '';
						$mdDialog.hide();
					} else {
	...
	$scope.UpdatePassword = function () {
		...
		if (!$scope.bpassword) {
			...
			var dataObj = $.param({
				password: $scope.bpassword.old,
				new_password: $scope.bpassword.newpassword,
				c_new_password: $scope.bpassword.c_newpassword
			});
		}
			...
			.then(
				function (response) {
					...
					if (response.data.success == true) {
						globals.mdToast('success', response.data.message);
						$scope.bpassword.old='';
						$scope.bpassword.newpassword = '';
						$scope.bpassword.c_newpassword = '';
						$mdDialog.hide();
					} else {
						globals.mdToast('error', response.data.message);
					}
