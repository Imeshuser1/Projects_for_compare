function Purchases_Controller($scope, $http, $mdSidenav, $q, $timeout) {
	...
	$scope.get_units();
	$scope.get_warehouse();
	$http.get(BASE_URL + 'api/table_columns/' + 'purchases').then(function (Data) {
	...
	$scope.updateColumns = function (column, value) {
		...
		var posturl = BASE_URL + 'api/update_columns/purchases';
	...
	$scope.sub_val = function (item) {
		var sub_val = 0;
		sub_val += item.quantity * item.price + ((item.tax) / 100 * item.quantity * item.price) - ((item.discount) / 100 * item.quantity * item.price);
		if (isNaN(sub_val)) {
			return 0;
		}
		else {
	...
	$scope.saveAll = function () {
		...
		/*Purchase to lower key purchases*/
...
function Purchase_Controller($scope, $http, $mdSidenav, $mdDialog, $q, $timeout) {
			...
			.then(
				function (response) {
					if (response.data.status === true) {
						...
						$scope.purchase.mail_status= response.data.sent_on;
					}
	...
	$http.get(BASE_URL + 'purchases/get_purchase/' + PURCHASEID).then(function (PurchaseDetails) {
		PurchaseDetails.data.items.forEach(function (item, index) {
			PurchaseDetails.data.items[index].price = Number(item.price);
			PurchaseDetails.data.items[index].tax = Number(item.tax);
			PurchaseDetails.data.items[index].quantity = Number(item.quantity);
			PurchaseDetails.data.items[index].discount = Number(item.discount);
		});
		...
		$scope.saveAll = function () {
			var EndRecurring;
			if ($scope.purchase.end_recurring) {
				EndRecurring = moment($scope.purchase.end_recurring).format("YYYY-MM-DD 00:00:00");
			} else {
				EndRecurring = 'Invalid date';
			}
			...
			var dataObj = $.param({
				...
				end_recurring:  $scope.purchase.end_recurring,
