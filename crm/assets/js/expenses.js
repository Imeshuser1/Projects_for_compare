function Expenses_Controller($scope, $http, $mdSidenav, $mdDialog, $q, $timeout, $filter) {
	...
	$scope.updateColumns = function (column, value) {
		...
		var posturl = BASE_URL + 'api/update_columns/expenses';
	...
	$http.get(BASE_URL + 'api/table_columns/' + 'expenses').then(function (Data) {
	...
	$scope.AddExpense = function () {
		...
		angular.forEach($scope.custom_fields, function (value) {
			...
			if (value.type === 'date') {
				...
				$scope.field_data = moment(datepicker,"yyyy-mm-dd").format().split("T")[0];
			}
...
function Expense_Controller($scope, $http, $mdSidenav, $mdDialog, $q, $timeout, fileUpload) {
	...
	$http.get(BASE_URL + 'expenses/get_expense/' + EXPENSEID).then(function (Expense) {
		Expense.data.items.forEach(function (item, index) {
			Expense.data.items[index].price = Number(item.price);
			Expense.data.items[index].tax = Number(item.tax);
			Expense.data.items[index].quantity = Number(item.quantity);
		});
		...
		$scope.UpdateExpense = function () {
			...
			var EndRecurring;
			if ($scope.expense.end_recurring) {
				EndRecurring = moment($scope.expense.end_recurring).format("YYYY-MM-DD 00:00:00");
			} else {
				EndRecurring = 'Invalid date';
			}
			...
			if (!$scope.expense) {
				...
				//	end_recurring: EndRecurring,
					end_recurring:  $scope.expense.end_recurring,
