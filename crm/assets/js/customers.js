function Customers_Controller($scope, $http, $mdSidenav, $filter, $mdDialog, fileUpload, $q) {
	...
	$scope.showPhoneError = function(event) {
	...
	$http.get(BASE_URL + 'api/table_columns/' + 'customers').then(function (Data) {
	...
	$scope.updateColumns = function(column, value) {
		...
		var posturl = BASE_URL + 'api/update_columns/customers';
	...
	$http.get(BASE_URL + 'api/customers').then(function (Customers) {
		...
		// $scope.countries = Customers.data.countries;
		...
		$scope.SameAsCustomerAddress = function () {
			...
			$scope.customer.billing_state_id = $scope.customer.state_id;
		...
		$scope.SameAsBillingAddress = function () {
			...
			$scope.customer.shipping_state_id= $scope.customer.billing_state_id;
		...
		$scope.updateDropdown = function (_prop) {
			...
			$http.get(BASE_URL + 'api/getCustomersCountry/'+_prop).then(function (response) {
...
function Customer_Controller($scope, $http, $filter, $mdSidenav, $mdDialog) {
	...
	$http.get(BASE_URL + 'customers/get_customer/' + CUSTOMERID).then(function (Customer) {
		...
		$http.get(BASE_URL + 'api/invoices').then(function (Invoices) {
			$scope.all_invoices = Invoices.data.data;
