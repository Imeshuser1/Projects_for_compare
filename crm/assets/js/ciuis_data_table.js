...
function Ciuis_Controller($scope, $http, $mdSidenav, $filter, $interval, $mdDialog, $element, $mdToast, $log) {
	...
	$http.get(BASE_URL + 'api/user').then(function (Userman) {
		...
		$scope.ChangeOnBoarding = function (id, value) {
			$http.post(BASE_URL + 'staff/onboarding_status/' + id + '/' + value)
			.then(
				function (response) {
					console.log(response);
				},
	...
	$http.get(BASE_URL + 'api/customers').then(function (Customers) {
		$scope.all_customer = Customers.data;
	});

	$scope.get_units = function() {
		$http.get(BASE_URL + 'api/get_units').then(function (Units) {
			$scope.units = Units.data;
		});
	}

	$scope.get_warehouse = function() {
		$http.get(BASE_URL + 'api/get_warehouses').then(function(Warehouses) {
  ...
  globals.get_stats = function() {
	...
	$scope.markAllAsRead = function() {
		...
		.then(function (response) {
			$http.get(BASE_URL + 'api/notifications').then(function (Notifications) {
				$scope.notifications = Notifications.data;
				$scope.notifications.markread=1;
				$scope.settings.newnotification = false;
			});
			$scope.stats.tbs = '0';
		}, function (error) {}
		);
	...
	function resetSearch() {
		...
		$scope.searchInventory.length = 0;
	}
					...
					function (response) {
						...
						if (response.data.length > 0) {
							...
							for (var i = 0; i < response.data.length; i++) {
								...
								if (response.data[i].type == 'inventory') {
									$scope.searchInventory = response.data[i].result;
								}
							}
						} else {
	...
	$scope.startTimer = function(action, id = null) {
		...
		if (action == 'stop') {
			...
			var posturl = BASE_URL + 'api/get_timer_list?id='+id;
