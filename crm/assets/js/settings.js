function Settings_Controller($scope, $http, $mdDialog, $mdSidenav, $sce, fileUpload, $q) {
	...
	$http.get(BASE_URL + 'api/get_modules').then(function (Modules) {
		$scope.modules = Modules.data;
		$scope.modules.forEach(function (item) {
			if (item.name == 'mollie') {
				$scope.isMollieActivated = (item.status == '1' ? true : false);
				$scope.MollieUsing = item.version;
			}

			if (item.name == 'hrm') {
				$scope.isHrmActivated = (item.status == '1' ? true : false);
				$scope.HrmUsing = item.version;
			}
		});
	});

	$scope.CheckModuleUpdate = function(module) {
		console.log(module);
	};
	$scope.GetModuleVersion = function(module) {
		console.log(module);
		$http.get(BASE_URL + 'modules/get_version/'+module).then(function(Data) {
			$scope.version = Data.data;
			if (module == 'mollie') {
				$scope.MollieUsing = $scope.version;
			}

			if (module == 'hrm' ) {
				$scope.HrmUsing = $scope.version;
			}
		});
	};

	$scope.isMollieAvailable = false;
	$scope.isMollieUptoDate = false;
	$scope.isMollieActivated = false;
	...
	$scope.deactivateModuleToggle = function (ev) {
		$scope.type = ev;
		$mdDialog.show({
			templateUrl: 'deactivate_module.html',
			scope: $scope,
			preserveScope: true,
			targetEvent: ev
		});
	...
	$scope.activateModule = function(type) {
		...
		$http.post(BASE_URL + 'modules/activate_module/'+ type, dataObj, config)
		.then(
			function(response) {
				$scope.checkingModule = false;
	...
	$scope.deactivateModule = function (type) {
		$scope.checkingModule = true;
		$http.post(BASE_URL + 'modules/deactivate_module/' + type, {}, config)
			.then(
				function (response) {
					$scope.checkingModule = false;
					if (response.data.success == true) {
	...
	$scope.saveCustomization = function() {
		...
		var dataObj = {
			...
			client_area_button_title: $scope.rebrand.client_area_button_title,
			enable_client_area_button: ($scope.rebrand.enable_client_area_button == true) ? '1' : '0',
			email_banner: $scope.banner_img,
		};
	...
	$scope.downloadAndInstallUpdate = function(ev) {
		if (LATEST_VERSION) {
			$mdDialog.show({
			...
			$http.post(BASE_URL + 'settings/download_and_install_update', dataObj, config).then(function(response) {
				if (response.data.success) {
					...
					$scope.installing = false;
					globals.mdToast('error', response.data.error);
					$mdDialog.hide();
				}
			});
		} else {
	...
	//HR Module
	let LATEST_VERSION_MODULE = null;
	$scope.CheckModuleVersion = function (module, ev) {
		$scope.updated = false;
		$mdDialog.show({
			templateUrl: 'version-check-module-template.html',
			scope: $scope,
			preserveScope: true,
			targetEvent: ev
		});
			$http.get(BASE_URL + 'modules/check_version/' + module).then(function (Version) {
			$scope.module = module;
			$scope.Version_detail = Version.data.name;
			$scope.Version_latest = Version.data.version;
			$scope.updated = false;
			$scope.last_updated = Version.data.last_updated;
		}, function (error) {
			showToast(NTFTITLE, update_error, ' danger');
		});
	};
	$scope.checkForModuleUpdates = function(ev) {
		$mdDialog.show({
			templateUrl: 'checking.html',
			parent: angular.element(document.body),
			clickOutsideToClose: false,
		...
		$http.get(BASE_URL + 'modules/check_version/' + $scope.module,null,config).then(function(Version) {
			$scope.Version_detail = Version.data.name;
			$scope.Version_latest = Version.data.version;
			if ($scope.Version_latest) {
				LATEST_VERSION_MODULE = $scope.Version_latest;
			}
			$scope.msg = Version.data.msg;
			$scope.updated = Version.data.updated;
			$scope.changeLog = $sce.trustAsHtml(Version.data.version_changelog);
	...
	$scope.install_update = false;
	$scope.downloadModuleUpdate = function(ev, module) {
		if (LATEST_VERSION_MODULE) {
			$mdDialog.show({
				templateUrl: 'updating.html',
				parent: angular.element(document.body),
				clickOutsideToClose: false,
			...
			var dataObj = $.param({
				version_number: LATEST_VERSION_MODULE
			});
			var config = {
				headers: {
					'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'
			...
			$http.post(BASE_URL + 'modules/download_update_module/'+ $scope.module, dataObj, config).then(function(response) {
				if (response.data.success) {
					$scope.install_update = true;
					$mdDialog.show({
						templateUrl: 'install-update-module.html',
						scope: $scope,
						preserveScope: true,
						targetEvent: ev
					});
	...
	$http.get(BASE_URL + 'api/settings_detail').then(function(Settings) {
		...
		if ($scope.settings_detail.error_reporting) {
			$scope.settings_detail.error_reporting = true;
		} else {
			$scope.settings_detail.error_reporting = false;
		}
		...
		$scope.UpdateSettings = function() {
			...
			var dataObj = $.param({
				...
				//error_reporting "E_ALL"
				error_reporting:$scope.settings_detail.error_reporting,
				...
				inventory_series: $scope.finance.inventory_series,
				inventory_prefix: $scope.finance.inventory_prefix,
				...
				payroll_prefix: $scope.finance.payroll_prefix,
				payroll_series: $scope.finance.payroll_series,
				payslip_prefix: $scope.finance.payslip_prefix,
				payslip_series: $scope.finance.payslip_series,
				warehouse_prefix: $scope.finance.warehouse_prefix,
				warehouse_series: $scope.finance.warehouse_series,
				header: editors.header.getValue() ? editors.header.getValue() : '',
				footer: editors.footer.getValue() ? editors.footer.getValue() : '',
				custom_css: editors.custom_css.getValue() ? editors.custom_css.getValue() : '',
			});
			var config = {
				headers: {
					'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'
	...
	$scope.system = {};
	$scope.settings.loader = true;
	$scope.system_loaded = false;
	$scope.load_system = function () {
		if ($scope.system_loaded) {
			return false;
		}
		$http.get(BASE_URL + 'settings/get_custom_files').then(function (system) {
			$scope.system = system.data;
			editors = {
				header: ace.edit('ace_header', {
					mode: 'ace/mode/html',
					// theme: 'ace/theme/twilight',
				}),
				footer: ace.edit('ace_footer', {
					mode: 'ace/mode/html',
					// theme: 'ace/theme/twilight',
				}),
				custom_css: ace.edit('ace_custom_css', {
					mode: 'ace/mode/css'
				}),
			};
			editors.header.setTheme('ace/theme/twilight');
			editors.footer.setTheme('ace/theme/twilight');
			editors.custom_css.setTheme('ace/theme/twilight');
			editors.header.session.setValue($scope.system.header);
			editors.footer.session.setValue($scope.system.footer);
			editors.custom_css.session.setValue($scope.system.custom_css);
			$scope.settings.loader = false;
			$scope.system_loaded = true;
		});
	};
	...
	$http.get(BASE_URL + 'api/get_appconfig/').then(function(Data) {
		...
		$scope.finance.payroll_series = ($scope.finance.payroll_series) ? (+$scope.finance.payroll_series) : '';
		$scope.finance.payslip_series = ($scope.finance.payslip_series) ? (+$scope.finance.payslip_series) : '';
		$scope.finance.inventory_series = ($scope.finance.inventory_series) ? (+$scope.finance.inventory_series) : '';
		$scope.finance.warehouse_series = ($scope.finance.warehouse_series) ? (+$scope.finance.warehouse_series) : '';
	});
