...
function Vendors_Controller($scope, $http, $mdSidenav) {
	...
	$http.get(BASE_URL + 'api/table_columns/' + 'vendors').then(function (Data) {
	...
	$scope.updateColumns = function(column, value) {
		...
		var posturl = BASE_URL + 'api/update_columns/vendors';
	...
	$http.get(BASE_URL + 'vendors/get_vendors').then(function (Vendors) {
		...
		$scope.byCountry = Vendors.data.countries;
		...
		$scope.updateDropdown = function (_prop) {
			...
			if(_prop == 'all'){
				...
				$http.get(BASE_URL + 'vendors/get_vendors/'+_prop).then(function (Vendors) {
				$scope.vendors = Vendors.data.data;
			}); }
		};
	});
...
function Vendor_Controller($scope, $http, $filter, $mdSidenav, $mdDialog) {
	...
	//logging the purchase in vendor 
	$http.get(BASE_URL + 'api/purchases').then(function (Purchase) {
		$scope.all_purchases = Purchase.data;
		$scope.purchases = $filter('filter')($scope.all_purchases, {
			vendor_id: VENDORRID,
		});
	});

	$scope.GoPurchases = function (index) {
		var purchase = $scope.purchases[index];
		window.location.href = BASE_URL + 'purchases/purchase/' + purchase.id;
	};
