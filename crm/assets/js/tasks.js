function Tasks_Controller($scope, $http, $mdSidenav, $filter) {
	...
	$scope.updateColumns = function(column, value) {
		...
		var posturl = BASE_URL + 'api/update_columns/tasks';
	...
	$http.get(BASE_URL + 'api/table_columns/' + 'tasks').then(function (Data) {
	...
	$scope.dropSuccessHandler = function ($event, index, array) {
		$scope.taskDropLoader = true;
		const tasks = [...$scope.tasks];
			tasks.splice(index, 1);
			$scope.tasks = tasks;
		//	lib.url.bind();
		// $scope.selected_task = $scope.tasks[index];
		// $scope.tasks.splice($scope.tasks.indexOf($scope.selected_task), 1);
	};

	$scope.onDrop = function ($event, $data, priority) {
		$scope.moved_task = $data;
		var dataObj = $.param({
			task_id: $scope.moved_task.id,
			priority_id: priority,
		});
		$http.post(BASE_URL + 'tasks/move_task/', dataObj, config)
		.then(
			function (response) {
				$http.get(BASE_URL + 'tasks/get_tasks').then(function (Tasks) {
					$scope.tasks = Tasks.data;
					$scope.taskDropLoader = false;
				//	lib.url.bind();
				});
			},
			function () {}
				...
				.then(
					function (response) {
						if(response.data.success == true) {
							...
							$http.get(BASE_URL + 'tasks/get_task/' + TASKID).then(function (Task) {
								$scope.task = Task.data;
								$scope.milestones = Task.data.milestones;
							});
						} else {
							$mdSidenav('Update').close();
							globals.mdToast('error', response.data.message );
