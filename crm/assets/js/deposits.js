function Deposits_Controller($scope, $compile, $http, $mdSidenav, $mdDialog, $q, $timeout, $filter) {
	...
	$http.get(BASE_URL + 'api/table_columns/' + 'deposits').then(function (Data) {
	...
	$scope.updateColumns = function (column, value) {
		...
		var posturl = BASE_URL + 'api/update_columns/deposits';
	...
	$scope.AddDeposit = function () {
		...
		if ($scope.newdeposit.date) {
            ...
            let datepicker = document.getElementById("input_39").value
            $scope.newdeposit.date = moment(datepicker,"yyyy-mm-dd").format().split("T")[0];
        }
...
function Deposit_Controller($scope, $http, $mdSidenav, $mdDialog, $q, $timeout) {
	...
	$http.get(BASE_URL + 'deposits/get_deposit/' + DEPOSITID).then(function (Deposit) {
		Deposit.data.items.forEach(function (item, index) {
			Deposit.data.items[index].price = Number(item.price);
			Deposit.data.items[index].tax = Number(item.tax);
			Deposit.data.items[index].quantity = Number(item.quantity);
		});
