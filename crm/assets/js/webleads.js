...
function WebLead_Controller($scope, $http, $mdSidenav, $mdDialog, $filter) {
	...
	$scope.get_staff();
	$scope.close = function () {
		$scope.weblead.custom_css = editors.custom_css.getValue();
	...
	$scope.viewIntegration = function (ev) {
		...
		setTimeout(function(){ 
			...
			if ($scope.weblead.custom_css == null) {
				$scope.weblead.custom_css = '';
			}
			editors.custom_css.session.setValue($scope.weblead.custom_css);
			editors.custom_css.getSession().on('change', function() {
				load_custom_css_preview(editors.custom_css.getValue());
			});
		}, 1000);
	};
	...
	$http.get(BASE_URL + 'leads/get_weblead/'+FORMID).then(function (WebLeads) {
		...
		$scope.webleadsLoader = false;
		load_custom_css_preview($scope.weblead.custom_css);
			...
			$('#saveFormContent').on('click', function() {
				...
				if (!hasError) {
					...
					var dataObj = $.param({
						...
						custom_css: $scope.weblead.custom_css,
					});
...
function load_custom_css_preview(css) {
	document.getElementById('custom_css_preview').innerHTML = css;
}
