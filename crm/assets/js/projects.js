	...
	$http.get(BASE_URL + 'projects/get_projects').then(function (Projects) {
		...
		$scope.CreateNew = function () { 
			...
			if (!$scope.project) {
				...
				if ($scope.project.start) {
					...
					let datepicker = document.getElementById("input_30").value
					$scope.project.start = moment(datepicker,"yyyy-mm-dd").format().split("T")[0];
				}
				...
				if ($scope.project.deadline) {
					...
					let datepicker = document.getElementById("input_32").value;
					$scope.project.deadline = moment(datepicker,"yyyy-mm-dd").add(1, 'day').format().split("T")[0]
		...
		$scope.copyProjectConfirm = function () {
			...
			if (!projectId) {
				...
				/*changing copyProject to copyproject*/
				var posturl = BASE_URL + 'projects/copy_project/' + projectId;
				$http.post(posturl, dataObj, config)
					.then(
						function (response) {
							if (response.data.success == true) {
...
function Project_Controller($scope, $http, $mdSidenav, $q, $timeout, $mdDialog, $filter, $sce, fileUpload) {
	...
	$http.get(BASE_URL + 'projects/projectmilestones/' + PROJECTID).then(function (Milestones) {
		...
		$scope.savingMilestone = $scope.loadingMilestones = false;
		$scope.UpdateMilestone = function (index) {
			$scope.savingMilestone = $scope.loadingMilestones = true;
			var milestone = $scope.milestones[index];
