function Reports_Controller($scope, $http) {
	...
	$scope.viewReport = function() {
		...
		$scope.timesheetReport = 'true';
		/*Url to lower case*/
		var posturl = BASE_URL + 'report/view_reports';
			$http.post(posturl, dataObj, config).then(function (response) {
				if (response.data.success == true) {
					if ( response.data.exporttype == 'invoices') {
						...
						$scope.loadingReport = false;
					} else if ( response.data.exporttype == 'tasktimer') {
						$scope.timesheet_list = {
							order: '',
							limit: 5,
							page: 1
						};
						$scope.timesheetReport = 'false';
						$scope.timesheets = response.data.result.timesheet;
						$scope.totalTime = response.data.result.total;
						$scope.timesheet_limitOptions = [5, 10, 15, 20];
						if ($scope.timesheets.length > 20) {
							$scope.timesheet_limitOptions = [5, 10, 15, 20, $scope.timesheets.length];
						}
						$scope.loadingReport = false;
					}
				} else {
	...
	/*Url to lower case*/
		var posturl = BASE_URL + 'report/export_data';
