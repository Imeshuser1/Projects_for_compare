...
class Vendors extends CIUIS_Controller {
	...
	function create() {
		if ( $this->Privileges_Model->check_privilege( 'vendors', 'create' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
					$this->form_validation->set_rules('name', lang('vendor'), 'trim|required|max_length[50]');
					$this->form_validation->set_rules('email', lang('email'), 'trim|required|max_length[50]|is_unique[vendors.email]');
					$this->form_validation->set_rules('groupid', lang('vendor'). ' ' .lang('group'), 'trim|required|integer');
					$this->form_validation->set_rules('taxoffice', lang('office'), 'trim|max_length[100]');
					$this->form_validation->set_rules('taxnumber', lang('number'), 'trim|max_length[40]');
					$this->form_validation->set_rules('executive', lang('Customer Authority'), 'trim|max_length[50]');
					$this->form_validation->set_rules('phone', lang('phone'), 'trim|max_length[20]');
					$this->form_validation->set_rules('fax', lang('fax'), 'trim|max_length[20]');
					$this->form_validation->set_rules('web', lang('web'), 'trim|max_length[100]');
					$this->form_validation->set_rules('city', lang('city'), 'trim|max_length[50]');
					$this->form_validation->set_rules('town', lang('town'), 'trim|max_length[50]');
					$this->form_validation->set_rules('zipcode', lang('zipcode'), 'trim|max_length[20]');
					$this->form_validation->set_rules('address', lang('address'), 'trim|max_length[100]');
					$data['message'] = '';
			...
			if ( isset( $vendor[ 'id' ] ) ) {
				if ( isset( $_POST ) && count( $_POST ) > 0 ) {
					if ( $this->Privileges_Model->check_privilege( 'vendors', 'edit' ) ) {
						$this->form_validation->set_rules('name', lang('vendor'), 'trim|required|max_length[50]');
						$this->form_validation->set_rules('email', lang('email'), 'trim|required|max_length[50]');
						$this->form_validation->set_rules('groupid', lang('vendor'). ' ' .lang('group'), 'trim|required|integer');
						$this->form_validation->set_rules('taxoffice', lang('office'), 'trim|max_length[100]');
						$this->form_validation->set_rules('taxnumber', lang('number'), 'trim|max_length[40]');
						$this->form_validation->set_rules('executive', lang('Customer Authority'), 'trim|max_length[50]');
						...
						$this->form_validation->set_rules('address', lang('address'), 'trim|max_length[100]');
							...
							if ($this->form_validation->run() == false) {
								...
								$email = $this->input->post('email');
								$primaryEmail = $this->Vendors_Model->get_vendors($id);
								$data['success'] = false;	
									if ($email != $primaryEmail['email']) {
										if ($this->Vendors_Model->isDuplicate($email) == TRUE) {
											$data['message'] = lang('Vendors email already exists');
												$hasError= true;
											echo json_encode($data);
										}else{
											$hasError = false;
	...
	function add_group() {
		if ( $this->Privileges_Model->check_privilege( 'vendors', 'create' ) ) {
			if (isset($_POST)) {
				$this->form_validation->set_rules('name', lang('name'), 'trim|required|max_length[25]');
				$data['message'] = '';
				if ($this->form_validation->run() == false) {
					$data['success'] = false;
					$data['message'] = validation_errors();
	...
	function remove( $id ) {
		...
		if($vendor) {
			if ( $this->Privileges_Model->check_privilege( 'vendors', 'delete' ) ) {
					if ( isset( $vendor[ 'id' ] ) ) {
						if($this->Vendors_Model->delete_vendors( $id, get_number('vendors',$id,'vendor','vendor'))){
							$data['success'] = true;
							$data['message'] = lang('vendor').' '.lang('deletemessage');
							echo json_encode($data);
						} else{
							$data['success'] = false;
							$data['message'] = lang('vendor').' '.lang('cannot_delete');
							echo json_encode($data);
						}
					} else {
	...
	function get_vendor( $id ) {
		...
		if($vendor) {
			$vendor_logs = $this->Logs_Model->vendor_logs($id);
			...
			$data_vendorlog = array();
			foreach ($vendor_logs as $logs) {
				$data_vendorlog[] = array(
	...
	function get_vendors($countryID="") {
		$vendors = array();
		if ( $this->Privileges_Model->check_privilege( 'vendors', 'all' ) ) {
			if(isset($countryID)){
				$vendors = $this->Vendors_Model->get_all_vendors_by_privileges("",$countryID);
			} else{
		...
		foreach ( $vendors as $vendor ) {
			...
			if(!isset($tempCountries)){
				$tempCountries[] = $vendor['country_id'];
			}
			
			if(!in_array($vendor['country_id'],$tempCountries)){
				array_push($tempCountries, $vendor['country_id']);
			}
