...
class Editor extends CIUIS_Controller {
	...
	function restore_default($type = null) {
		switch ($type) {
			case '1':
				$data = file_get_contents(APPPATH. 'language/'.$this->session->userdata('language').'/'.$this->session->userdata('language').'_default_lang.php');
				if ($data == false) {
	...
	function get_data($type = null) {
		switch ($type) {
			...
			case '4':
				...
				break;
			case '5':
				$data = file_get_contents(APPPATH. 'views/proposals/pdf.php');
				if ($data == false) {
					$data = lang('unable_to_read');
				}
				break;
			case '6':
				$data = file_get_contents(APPPATH. 'views/orders/pdf.php');
				if ($data == false) {
					$data = lang('unable_to_read');
				}
				break;
			case '7':
				$data = file_get_contents(APPPATH. 'views/deposits/pdf.php');
				if ($data == false) {
					$data = lang('unable_to_read');
				}
				break;
			case '8':
				$data = file_get_contents(APPPATH. 'views/projects/pdf.php');
				if ($data == false) {
					$data = lang('unable_to_read');
				}
				break;
