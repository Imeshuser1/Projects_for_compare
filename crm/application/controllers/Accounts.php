...
class Accounts extends CIUIS_Controller {
	...
	function create() {
		if ( $this->Privileges_Model->check_privilege( 'accounts', 'create' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				...
				$this->form_validation->set_rules('bankname',  lang('account') .' '. lang('name'), 'trim|required|max_length[50]');
				if ($type == 1) {
					$this->form_validation->set_rules('bankname',  lang('bank') .' '. lang('name'), 'trim|required|max_length[50]');
					$this->form_validation->set_rules('branchbank', lang('branchbank'), 'trim|required|max_length[100]');
					$this->form_validation->set_rules('account',  lang('account') . ' '. lang('number'), 'trim|required|max_length[20]');
					$this->form_validation->set_rules('iban', lang("SWIFT / IBAN", 'Number'), 'trim|max_length[40]');
				} else {
					$this->form_validation->set_rules('bankname', lang('bank') .' '. lang('name'), 'trim|max_length[50]');
					$this->form_validation->set_rules('branchbank', lang('branchbank'), 'trim|max_length[100]');
					$this->form_validation->set_rules('account',lang('account') . ' '. lang('number'), 'trim|max_length[20]');
					$this->form_validation->set_rules('iban', lang('iban'), 'trim|max_length[40]');
				}
		...
		if ( $this->Privileges_Model->check_privilege( 'accounts', 'all' ) ) {
			if ( $this->Privileges_Model->check_privilege( 'accounts', 'edit' ) ) {
				...
				if ( isset( $data[ 'accounts' ][ 'id' ] ) ) {
					if ( isset( $_POST ) && count( $_POST ) > 0 ) {
							$this->form_validation->set_rules('name',lang('account') .' '. lang('name'), 'trim|required|max_length[50]');
							$this->form_validation->set_rules('bankname',  lang('bank') .' '. lang('name'), 'trim|max_length[50]');
							$type = $this->input->post('type');
							if ($type == 1) {
								$this->form_validation->set_rules('bankname',  lang('bank') .' '. lang('name'), 'trim|max_length[50]|required');
								$this->form_validation->set_rules('branchbank', lang('branchbank'), 'trim|max_length[20]|required');
								$this->form_validation->set_rules('account', lang('account') . ' '. lang('number'), 'trim|max_length[20]|required');
							} else {
								$this->form_validation->set_rules('branchbank', lang('Branch Bank'), 'trim|max_length[100]');
								$this->form_validation->set_rules('account',lang('account') . ' '. lang('number'), 'trim|max_length[20]');
								$this->form_validation->set_rules('iban', lang('iban'), 'trim|max_length[40]');
							}
							if ($this->form_validation->run() == false) {
		...
		if ( $this->Privileges_Model->check_privilege( 'accounts', 'all' ) ) {
			...
			foreach ( $payments as $payment ) {
				if ( $payment[ 'customer_id' ] != 0 ){
					...
					$name = "";
					switch (!is_array( $customer[ 'type' ] ?? false )) {
						case '0':
				...
				$payments_data[] = array(
					...
					'date' => date( get_dateTimeFormat(), strtotime( $payment[ 'date' ] ) ),
	...
	function get_accounts() {
		if ( $this->Privileges_Model->check_privilege( 'accounts', 'all' ) ) {
			$accounts_not_actives = $this->Accounts_Model->get_not_active_accounts();
			$data_account = array();
			foreach ($accounts_not_actives  as $account ) {
				switch ( $account[ 'type' ] ) {
					case '0':
