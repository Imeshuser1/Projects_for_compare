...
class Leads extends CIUIS_Controller
{
	...
	function save_weblead_components($id)
	{
		if ($this->Privileges_Model->check_privilege('leads', 'edit')) {
			if (isset($_POST) && count($_POST) > 0) {
				$weblead_id = $this->db->where('id', $id)->update('webleads', array(
					'form_data' => $this->input->post('components', FALSE),
					'custom_css' => $this->input->post('custom_css', FALSE),
				));
				if ($weblead_id) {
					$data['success'] = true;
					$data['message'] = lang('weblead') . ' ' . lang('updatemessage');
	...
	function webleads()
	{
		...
		foreach ($leads as $lead) {
			...
			$data_leads[] = array(
				...
				'createddate' => date(get_dateTimeFormat(), strtotime($lead['created'])),
				'status' => $status
			);
		};
	...
	function create()
	{
		if ($this->Privileges_Model->check_privilege('leads', 'create')) {
			if (isset($_POST) && count($_POST) > 0) {
				$this->form_validation->set_rules('title', lang('title'), 'trim|required|max_length[50]');
				...
				$this->form_validation->set_rules('assigned', lang('sssigned', 'Staff'), 'trim|required|integer');
				$this->form_validation->set_rules('status', lang('status'), 'trim|required|integer');
				$this->form_validation->set_rules('source', lang('source'), 'trim|required|integer');
				$this->form_validation->set_rules('phone', lang('phone'), 'trim|max_length[20]');
				$this->form_validation->set_rules('email', lang('email'), 'trim|required|max_length[50]|is_unique[leads.email]', array('is_unique' => lang("E-Mail address already exist.")));
				$this->form_validation->set_rules('website', lang('customer_web'), 'trim|max_length[100]');
				$this->form_validation->set_rules('country_id', lang('country'), 'trim|required|integer');
				$this->form_validation->set_rules('state_id', lang('state'), 'trim|integer');
				$this->form_validation->set_rules('city', lang('city'), 'trim|max_length[50]');
				$this->form_validation->set_rules('zip', lang('post_code'), 'trim|max_length[15]');
				...
				$this->form_validation->set_rules('type', lang('type'), 'trim|integer|in_list[0,1]');
				$this->form_validation->set_rules('public', lang('public'), 'trim|integer|in_list[0,1]');
				$this->form_validation->set_rules('date_contacted', lang('date_contacted'), 'trim|required|valid_date_time');
				$this->form_validation->set_rules('tags', lang('tags'), 'trim|max_length[100]');
			...
			if ($this->Privileges_Model->check_privilege('leads', 'edit')) {
				if (isset($data['lead']['id'])) {
					if (isset($_POST) && count($_POST) > 0) {
						...
						$this->form_validation->set_rules('name', lang('name'), 'trim|required|max_length[50]');
						$this->form_validation->set_rules('company', lang('company'), 'trim|max_length[50]');
						$this->form_validation->set_rules('assigned_id', lang('assigned', 'staff'), 'trim|required|integer');
						$this->form_validation->set_rules('status', lang('status'), 'trim|required|integer');
						$this->form_validation->set_rules('source', lang('source'), 'trim|required|integer');
						$this->form_validation->set_rules('phone', lang('phone'), 'trim|max_length[20]');
						$this->form_validation->set_rules('email', lang('email'), 'trim|required|max_length[50]');
						$this->form_validation->set_rules('website', lang('cusotmer_web'), 'trim|max_length[100]');
						$this->form_validation->set_rules('country_id', lang('country'), 'trim|required|integer');
						$this->form_validation->set_rules('state_id', lang('state'), 'trim|integer');
						$this->form_validation->set_rules('city', lang('city'), 'trim|max_length[50]');
						$this->form_validation->set_rules('zip', lang('post_code'), 'trim|max_length[15]');
	...
	function convert($id)
	{
		if ($this->Privileges_Model->check_privilege('customers', 'create')) {
			...
			if ($lead['dateconverted'] != null) {
				...
				//Adding contact details when lead is converted to customer.
				$params = array(
					'name' => $lead['leadname'],
					'phone' => $lead['leadphone'],
					'email' => $lead['leadmail'],
				...
				$contacts_id = $this->db->insert('contacts', $params);
	...
	function get_lead($id)
	{
		...
		if ($lead) {
			...
			$data_lead = array(
				...
				'date_contacted' => $lead['date_contacted'] ? date(get_dateTimeFormat(), strtotime($lead['date_contacted'])) : '',
				'lost' => $lead['lost'],
				'junk' => $lead['junk'],
				'public' => $is_public,
				'type' => $is_individual,
	...
	function get_leads()
	{
		...
		foreach ($leads as $lead) {
			...
			$data_leads[] = array(
				...
				'lead_number' => get_number('leads', $lead['id'], 'lead', 'lead'),
				'email' => $lead['leadmail'],
				'leadclass' => ($lead['lost'] == 1 ? 'ion-sad text-black' : ($lead['junk'] == 0 ? '' : 'ion-sad-outline text-warning')),
				'value' => ($lead['lost'] == 1 ? lang('lost') : ($lead['junk'] == 0 ? '' : lang('junk'))),
			);
		};
		echo json_encode($data_leads);
	}
