...
class Tasks extends CIUIS_Controller {
	...
	function create() {
		if ( $this->Privileges_Model->check_privilege( 'tasks', 'create' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				$this->form_validation->set_rules('relation', lang('project'), 'trim|required|max_length[5]');
				...
				$this->form_validation->set_rules('relation_type', lang('relation_type'), 'trim|max_length[30]');
				...
				if ($this->input->post('billable') == 1) {
					$this->form_validation->set_rules('hourly_rate', lang('hourly_rate'), 'trim|required');
				}
				...
				//$this->form_validation->set_rules('status_id', lang('status'), 'trim|required|integer');
				...
				if ($this->form_validation->run() == false) {
					...
					$params = array(
						...
						'status_id' => '0', // Mark status as 'New'
	...
	{
		$created = strtotime($this->input->post('startdate'));
		$duedate = strtotime($this->input->post('duedate'));
		if ($duedate >= $created)
			return True;
		else {
			$this->form_validation->set_message('compareDate', lang('startdate').' '.lang('date_error'). ' ' .lang('duedate'));
			return False;
		}
	}
	function update( $id ) {
	...
	function move_task() {
		if ( $this->Privileges_Model->check_privilege( 'tasks', 'edit' ) ) {
			...
			$response = $this->db->where( 'id', $task_id )->update( 'tasks', array( 'priority' => $priority_id ) );
			$task = $this->Tasks_Model->get_task_detail( $task_id );
			switch ( $task[ 'status_id' ] ) {
				case '0':
					$status = lang( 'new' );
					break;
