...
class Staff extends CIUIS_Controller
{
	...
	function create_staff()
	{
		if ($this->Privileges_Model->check_privilege('staff', 'create')) {
			...
			if (isset($_POST) && count($_POST) > 0) {
				...
				$this->form_validation->set_rules('email', lang('email'), 'trim|required|max_length[50]|is_unique[staff.email]', array('is_unique' => lang("E-Mail address already exist.")));
				$this->form_validation->set_rules('password', lang('password'), 'trim|required|max_length[100]');
				$this->form_validation->set_rules('department', lang('department'), 'trim|required|integer');
				$this->form_validation->set_rules('language', lang('language'), 'trim|required|max_length[25]');
				$this->form_validation->set_rules('timezone', lang('timezone'), 'trim|required|max_length[50]');
				$this->form_validation->set_rules('role', lang('role'), 'trim|required|integer');
				...
				$this->form_validation->set_rules('state', lang('state'), 'trim|integer');
				$hasError = false;
				$data['message'] = '';
				...
				if (isset($id)) {
					if (isset($_POST) && count($_POST) > 0) {
						...
						if (!$is_demo) {
							...
							$this->form_validation->set_rules('language', lang('language'), 'trim|required|max_length[25]');
							$this->form_validation->set_rules('timezone', lang('timezone'), 'trim|required|max_length[50]');
							$this->form_validation->set_rules('role', lang('role'), 'trim|required|is_natural_no_zero');
							$this->form_validation->set_rules('phone', lang('phone'), 'trim|required|max_length[20]');
							$this->form_validation->set_rules('address', lang('address'), 'trim|max_length[100]');
							$this->form_validation->set_rules('state', lang('state'), 'trim|integer');
							$hasError = false;
							...
							if ($this->form_validation->run() == false) {
								...
								echo json_encode($data);
									...
									if (!$hasError) {
										...
										$is_Staff = '';
										$is_Other = '';
										if ($role_type == 'admin' || $role_type == 'sadmin') {
											$is_Admin = 1;
										} else if ($role_type == 'staff') {
											$is_Staff = 1;
										} else {
					...
					if (!$is_demo) {
						if ($staff['id'] == $this->session->usr_id) {
							...
							if ($result) {
								...
								$data['message'] = sprintf(lang('cant_delete_transaction_related_staff', $staff . ''));
								$data['success'] = false;
								echo json_encode($data);
							}
						}
	...
	function onboarding_status($id, $value)
	{
		if ($value === 'true') {
			$status = 1;
			$this->session->set_userdata('tour_status', $status);
		} else {
			$status = 0;
			unset($_SESSION['tour_status']);
		}
		if (isset($id)) {
			$response = $this->db->where('id', $id)->update('staff', array('tour_status' => $status));
		};
	}
		...
		if ($this->Privileges_Model->check_privilege('staff', 'all')) {
			...
			foreach ($staffs as $staff) {
				...
				if ($staff['role_type'] == 'admin') {
					$type = $staff['role_name'];
					$color = '#26c281';
				} elseif ($staff['role_type'] == 'staff') {
					$type = $staff['role_name'];
					$color = '#686868';
				} elseif ($staff['role_type'] == 'hr_manager') {
					$type = $staff['role_name'];
					$color = '#e8ab00';
				} elseif ($staff['role_type'] == 'other') {
					$type = $staff['role_name'];
					$color = '#e8ab00';
				}
				$data_staffs[] = array(
