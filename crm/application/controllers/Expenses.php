...
class Expenses extends CIUIS_Controller
{
	...
	function create()
	{
		if ($this->Privileges_Model->check_privilege('expenses', 'create')) {
			if (isset($_POST) && count($_POST) > 0) {
				...
				$this->form_validation->set_rules('internal', lang('internal'), 'trim|in_list[false,true]');
				$internal = $this->input->post('internal');
				$internal = ($internal == 'true') ? '1' : '0';
				if ($internal == '0') {
					$this->form_validation->set_rules('customer', lang('customer'), 'trim|required|integer');
				} else if ($internal == '1') {
					$this->form_validation->set_rules('staff', lang('staff'), 'trim|required|integer');
				}
				...
				$this->form_validation->set_rules('number', lang('description'), 'trim|max_length[50]');
				...
				if ($recurring == '1') {
					...
					$this->form_validation->set_rules('recurring_type', lang('Recurring Type'), 'trim|required|integer');
				}
				...
				if (!$this->session->userdata('other')) {
					if (isset($data['expenses']['id'])) {
						if (isset($_POST) && count($_POST) > 0) {
							$this->form_validation->set_rules('title', lang('title'), 'trim|required|max_length[50]');
							$this->form_validation->set_rules('category', lang('category'), 'trim|required|integer');
							$this->form_validation->set_rules('internal', lang('iinternal'), 'trim|in_list[false,true]');
							$internal = $this->input->post('internal');
							$internal = ($internal == 'true') ? '1' : '0';
							if ($internal == '0') {
								$this->form_validation->set_rules('customer', lang('customer'), 'trim|required|integer');
	...
	function convert($id)
	{
		if ($this->Privileges_Model->check_privilege('invoices', 'create')) {
			...
			if (isset($id)) {
				$customer= $this->Customers_Model->get_customers($expenses['customer_id']);
				$params = array(
					'token' => md5(uniqid()),
				...
				$items = $this->Report_Model->get_items('expense', $id);
	...
	function get_expense($id)
	{
		...
		if ($expense) {
			if($expense['invoice_id']){
				$items = $this->Report_Model->get_items('invoice', $expense['invoice_id']);
			}	else{
	...
	function get_expenses()
	{
		...
		foreach ($expenses as $expense) {
			...
			$data_expenses[] = array(
				...
				'color' => $color,
				'billable' => $billable,
				'customer' => $expense['customer_id'] ? ($expense['company'] ? $expense['company'] : $expense['individual']) : '',
				'staff' => $expense['staff'] ? $expense['staff'] : '',
				'date' => date(get_dateFormat(), strtotime($expense['date'])),
