...
class Customers extends CIUIS_Controller
{
	...
	function create()
	{
		if ($this->Privileges_Model->check_privilege('customers', 'create')) {
			if (isset($_POST) && count($_POST) > 0) {
				...
				$this->form_validation->set_rules('company', lang('company'), 'trim|required|max_length[50]');
				$namesurname = $this->input->post('namesurname');
				$this->form_validation->set_rules('email', lang('email'), 'trim|required|valid_email|max_length[50]');
				$this->form_validation->set_rules('phone', lang('phone'), 'trim|required');
				$this->form_validation->set_rules('default_payment_method', lang('Default Payment Method'), 'trim|max_length[30]');
				$this->form_validation->set_rules('groupid', lang('customer', 'group'), 'trim|required|integer');
				...
				if ($this->form_validation->run() == false) {
					...
					if ($this->Contacts_Model->isDuplicate($this->input->post('email'))) {
						$data['message'] = lang('Contact', 'email is already exist.');
					} else {
			...
			if (isset($customer['id'])) {
				if (isset($_POST) && count($_POST) > 0) {
					if ($this->Privileges_Model->check_privilege('customers', 'edit')) {
						...
						$this->form_validation->set_rules('email', lang('email'), 'trim|required|valid_email|max_length[50]');
						$this->form_validation->set_rules('phone', lang('phone'), 'trim|required');
						$this->form_validation->set_rules('default_payment_method', lang('default_payment_method'), 'trim|max_length[30]');
						if ($this->input->post('type') == 'true') {
							$type = 1;
							$company = '';
						} else {
			...
			if (!$this->upload->do_upload('file')) {
				...
				if ($this->import->get_array($file_path)) {
					...
					foreach ($csv_array as $row) {
						...
						$group_id = $group_name ? $this->get_group_id($group_name) : '';
						if (!empty($row['email']) && !empty($row['type'])){
							$num++;
							$csv_errors[] = array(
								'line' => $num,
							);
							...
							$insert_data = array(
								...
								'email' => $row['email'],
								'web' => $row['web'],
								'customer_status_id' => isset($row['customer_status_id']),
								'phone' => $row['phone'],
								'risk' => $row['risk'],
								'staff_id' => $this->session->userdata('usr_id'),
								'groupid' => $group_id,
	...
	function exportdata()
	{
		...
		if ($this->Privileges_Model->check_privilege('customers', 'all')) {
			$this->db->select('type, created, company, namesurname, taxoffice, taxnumber, ssn, executive, address, zipcode, country_id, state, city, town, phone, fax, email, web, customer_status_id, risk,customergroups.name as group_name');
	...
	function get_customer($id)
	{
		...
		if ($customer) {
			...
			$customer_logs = $this->Logs_Model->customer_logs($id);
			...
			$data_customerlog = array();
			foreach ($customer_logs as $logs) {
				$data_customerlog[] = array(
			...
			$data_customerdetail = array(
				...
				'customer_logs' => $data_customerlog,
