...
class Projects extends AREA_Controller {
	...
	function project( $id ) {
		$permission = $this->Projects_Model->check_project_permission( $id, $this->session->contact_id );
		if ($permission) {
	...
	function addnote() {
		if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				$this->form_validation->set_rules('relation_type', lang('relation_type'), 'trim|required|max_length[100]');
				$this->form_validation->set_rules('description', lang('description'), 'trim|required|max_length[65535]');
				$this->form_validation->set_rules('relation', lang('relation'), 'trim|required|interger');
				$data['message'] = '';
				...
				$data[ 'insert_id' ] = $this->db->insert_id();
					...
					if ($template['status'] == 1) {
						...
						$message_vars = array(
							...
							'{loggedin_staff}' => $this->session->name,
