...
class Login extends CI_Controller {
		...
		if ( $this->form_validation->run() == false ) {
			...
			if ( $this->Area_Model->resolve_user_login( $email, $password ) ) {
				...
				$this->session->set_userdata( array(
					...
					'is_admin' => $user->admin,
					'logged_in' => true,
					'cust_tour_status' => $user->tour_status
				) );
				//echo $_SESSION[ 'admin' ];
				redirect( 'area/panel' );
			} else {
	...
	function onboarding_status( $value ) {
		if ( $value === 'true' ) {
			$status = 1;
			$this->session->set_userdata('cust_tour_status', $status);
		} else {
			$status = 0;
			unset($_SESSION['cust_tour_status']);
		}
		$response = $this->db->where( 'id', $this->session->contact_id )->update( 'contacts', array( 'tour_status' => $status ) );
	}
}
