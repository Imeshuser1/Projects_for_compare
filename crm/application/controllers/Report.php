...
class Report extends CIUIS_Controller {
	...
	function index() { 
		...
		if(!($this->Privileges_Model->check_privilege('report', 'all'))){
			$this->session->set_flashdata( 'ntf3', '' . lang( 'you_dont_have_permission' ) );
			redirect( 'panel/' );
			die;
		}
...
/*function to lower case*/
	function view_Reports() {
		if ( isset( $_POST ) && count( $_POST ) > 0 ) {
			...
			if (!$hasError) {
				...
				} else if ( $exporttype == 'tasktimer') {
					$data['result'] = $this->Report_Model->TimesheetsReport(); 
				} 
	...
	function export_Data() {
		...
		} else if ( $type == 'tasktimer') {
			$this->db->select( 'tasktimer.id, tasktimer.start, tasktimer.end, tasktimer.task_id, tasks.name, tasktimer.note, tasks.task_number as task_number, staff.staffname as staff, staff.id as staff_id, staff.staffavatar as avatar, staff.email as staff_email, staff.staff_number as staff_number' );
		...
		/*** Get Current Year Report ***/
			...
			if( $type == 'contacts') {
				...
				$this->db->from($type)->where('YEAR('.$type.'.start)', $year);
			} else {
		...
		/*** Get Current Month Report ***/
			...
			if( $type == 'contacts') {
				...
				$this->db->from($type)->where('MONTH('.$type.'.start)', $month);
			} else {
		...
		/*** Get Current Week Report ***/
			...
			if( $type == 'contacts') {
				...
				$this->db->from($type)->where( 'WEEK('.$type.'.start) = WEEK(CURRENT_DATE)');
			} else {
		...
		/*** Get Last Year Report ***/
			...
			if( $type == 'contacts') {
				...
				$this->db->from($type)->where('YEAR('.$type.'.start) = YEAR(CURRENT_DATE - INTERVAL 1 YEAR)');
			} else {
		...
		/*** Get Last Month Report ***/
			...
			if( $type == 'contacts') {
				...
				$this->db->from($type)->where('MONTH('.$type.'.start) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH)');
			} else {
		...
		/*** Get Last Week Report ***/
			...
			if( $type == 'contacts') {
				...
				$this->db->from($type)->where('WEEK('.$type.'.start) = WEEK(CURRENT_DATE - INTERVAL 1 WEEK)');
			} else {
		...
		/*** Get Custom Report ***/
			...
			if( $type == 'contacts') {
				...
				$this->db->where(array($type.'.start>=' => $from, $type.'.start<=' => $to));
			} else {
		...
		if ($type == 'invoices') {
        	...
        	foreach ($q as $data) { 
				...
				$purchasesstatus = $this->Purchases_Model->get_status($data)['status'];
				$export[] = array(
        	...
        	foreach ($q as $data) {
        		...
        		$staff_number = $data['staff_number'] ? $data['staff_number'] : $data['staff_id'];
        		$task_number = $data['task_number'] ? $data['task_number'] : $data['task_id'];
	        	$export[] = array(
	        		lang('staff') => $data['staff'].'('.$staff_number.')',
	        		lang('type') => $data['name'] .'('. $task_number .')',
	        		lang('start_time') =>  date(get_dateTimeFormat(), strtotime($data['start'])),
	        		lang('end_time') => $data['end'] ? (date(get_dateTimeFormat(), strtotime($data['end']))):null,
	        		lang('timeCaptured') => $total,
	        	);
	        }
	        $filename = lang('timesheets');
        }
	...
	function generatePdf() {
		...
		if( $type == 'invoices' ) {
			...
			$data['results'] = $this->Report_Model->get_all_timesheet();
		}
		$filename = $type.'.pdf';
		$data['type'] = $type;
