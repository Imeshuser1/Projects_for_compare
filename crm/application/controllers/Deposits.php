	...
	function create()
	{
		if ($this->Privileges_Model->check_privilege('deposits', 'create')) {
			if (isset($_POST) && count($_POST) > 0) {
				...
				$this->form_validation->set_rules('recurring_status', lang('RECURRING'), 'trim|integer|in_list[0,1]');
				$recurring = $this->input->post('recurring_status');
				if ($recurring == '1') {
	...
	function get_deposits()
	{
		...
		foreach ($deposits as $deposit) {
			...
			$data_deposits[] = array(
				...
				'color' => $color,
				'customer' => $deposit['customer_id'] ? ($deposit['company'] ? $deposit['company'] : $deposit['individual']) : '',
				'staff' => $deposit['staff'] ? $deposit['staff'] : '',
				'date' => date(get_dateFormat(), strtotime($deposit['date'])),
