...
class Vendors_Model extends CI_Model {
	...
	function get_all_vendors_by_privileges($staff_id='', $countryID="") {
		$this->db->select( '*, vendors.id as id ' );
		$this->db->join('vendors_groups','vendors.groupid = vendors_groups.id','left');
		$this->db->order_by( 'vendors.id', 'desc' );
		if($staff_id) {
			if($countryID){
				return $this->db->get_where( 'vendors', array( 'staff_id' => $staff_id, 'country_id'=>$countryID ) )->result_array();
			} else{
			...
			if($countryID){
				return $this->db->get_where( 'vendors', array( 'country_id'=>$countryID ) )->result_array();	
			}else{
				return $this->db->get_where( 'vendors', array( '' ) )->result_array();
			}
