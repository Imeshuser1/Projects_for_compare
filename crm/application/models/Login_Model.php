...
class Login_Model extends CI_Model {
	...
	function set_session() {
		$this->session->set_userdata( array(
			...
			'LoginOK' => true,
			'tour_status' => $this->details->tour_status
		) );
		...
		if($connection['status']) {
			...
			if(!$verification['status']) {
				$this->session->set_userdata(['remote_check' => false, 'LoginOK' => false, 'admin' => '']);
			} else {
				$this->session->set_userdata(['remote_check' => true, 'LoginOK' => true]);
			}
		} else {
			$this->session->set_userdata(['remote_check' => true, 'LoginOK' => true]);
		}
