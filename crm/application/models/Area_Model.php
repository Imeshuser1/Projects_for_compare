...
class Area_Model extends CI_Model {
	...
	function ttc() {
		...
		$this->db->where( 'contact_id = ' . $this->session->contact_id . '' );
		$query = $this->db->get();
		$ttc = $query->num_rows();
		return $ttc;
	}
	...
	function otc() {
		$this->db->from( 'tickets' )->where( 'status_id = 1 AND contact_id = ' . $this->session->contact_id . '' );
		$query = $this->db->get();
		$otc = $query->num_rows();
		return $otc;
	}
	...
	function ipc() {
		$this->db->from( 'tickets' )->where( 'status_id = 2 AND contact_id = ' . $this->session->contact_id . '' );
		$query = $this->db->get();
		$ipc = $query->num_rows();
		return $ipc;
	}
	...
	function atc() {
		$this->db->from( 'tickets' )->where( 'status_id = 3 AND contact_id = ' . $this->session->contact_id . '' );
		$query = $this->db->get();
		$atc = $query->num_rows();
		return $atc;
	}
	...
	function ctc() {
		$this->db->from( 'tickets' )->where( 'status_id = 4 AND contact_id = ' . $this->session->contact_id . '' );
		$query = $this->db->get();
		$ctc = $query->num_rows();
		return $ctc;
	}
	...
	function add_tickets( $params ) {
		...
		$contactname = $this->session->name;
		$contactsurname = $this->session->surname;
		$loggedinuserid = $this->session->contact_id;
	...
	function newnotification() {
		$new = $this->db->where( 'customerread = "0" AND contact_id = ' . $this->session->contact_id . '' )->get( 'notifications' )->result();
		if ( $new ) {
			return true;
		} else {
			return false;
	...
	function get_all_notifications() {
		...
		$this->db->where( 'contact_id = ' . $this->session->contact_id . ' OR customer_id = ' . $this->session->customer . '' );
	...
	function customerdebt() {
		...
		$this->db->where( 'status_id = 3 AND customer_id = ' . $this->session->customer . '' );
		return $this->db->get()->row()->total;
	}

	function logs() {
		return $this->db->select( '*,,staff.staffname as staffmembername, staff.staffavatar as staffimage, logs.date as date ' )->join( 'staff', 'logs.staff_id = staff.id', 'left' )->order_by( "date", "desc" )->get_where( 'logs', array( 'customer_id' => $this->session->customer ) )->result_array();
	}
