...
class Search_Model extends CI_Model {
    ...
    function search_data($input) {
        ...
        foreach($methods as $method) {
            preg_match('/^search([A-Z][a-z]+)$/', $method, $matches);
            if($matches && $matches[1]) {
                $type = strtolower($matches[1]);
                $typeSingular = preg_replace('/s$/', '', $type);
                if($type == 'staff') {
                    $fixedType = 'staff_members';
                } else {
                    $fixedType = $type;
                }
                $searchMethods[$type] = [
                    'method' => $method,
                    'lang_type' => mb_strtolower(lang($fixedType)),
                    'lang_type_singular' => mb_strtolower(lang($typeSingular))
                ];
            }
        }
        $input = mb_strtolower($input);
        array_walk($searchMethods, function($searchMethod) use ($input, $limit, &$result) {
            if($input == $searchMethod['lang_type']) {
                $input = '';
                $limit = null;
            } elseif($input == $searchMethod['lang_type_singular']) {
                $input = '';
            }
            $items_search = $this->{$searchMethod['method']}($input, $limit);
    ...
    function searchInventory($q, $limit) {
        $result = [
            'result' => [],
            'type' => 'inventory',
        ];
        $isAdmin = $this->isAdmin();
        $user_id = $this->session->userdata('usr_id');
        $has_permission_view_inventories = $this->has_permission('inventories');
        if($has_permission_view_inventories) {
            ...
            if(!$isAdmin) {
                $this->db->where("product_inv.inventory_created_by = $user_id");
            }
            $this->db->where("(
                product_inv.inventory_id LIKE '$q%'
                OR product_inv.inventory_number LIKE '%$q%'
                OR products.productname LIKE '%$q%'
                OR product_inv.product_type LIKE '%$q%'
                OR productcategories.name LIKE '%$q%'
                OR warehouses.warehouse_name LIKE '%$q%'
                OR product_inv.stock_qty LIKE '%$q%'
                OR product_inv.cost_price LIKE '%$q%'
                OR product_inv.move_type LIKE '%$q%'
                OR staff.staffname LIKE '%$q%'
                OR product_movement.movement_name LIKE '%$q%'
            )");
