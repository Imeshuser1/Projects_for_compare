...
class Customers_Model extends CI_Model {
	...
	function get_all_customers_Country($staff_id='', $countryID='') {
		...
		if($staff_id) {
			if($countryID){
				//Get All Customer List Filtered By Staff & Country
				return $this->db->get_where( 'customers', array( 'staff_id' => $staff_id, 'country_id'=>$id) )->result_array();	
			
			}	else{
				//Get All Customer List only Filtered By Staff
				return $this->db->get_where( 'customers', array( 'staff_id' => $staff_id) )->result_array();
			}
			
		} else {
			if($countryID){
				// Get All Customer List Filtered by Country;
				return $this->db->get_where( 'customers', array( 'country_id'=>$countryID) )->result_array();	
	...
	function add_customers( $params ) {
		...
		if ($isContact == '1') {
			...
			if($contacts_id){
				$this->update_contact_privilege($contacts_id, 'true', '1');
				$this->update_contact_privilege($contacts_id, 'true', '2');
				$this->update_contact_privilege($contacts_id, 'true', '7');
				$this->update_contact_privilege($contacts_id, 'true', '9');
				$this->update_contact_privilege($contacts_id, 'true', '16');
...
/*group deletion*/
	...
	function delete_customers( $id, $number, $customer_id = null) {
