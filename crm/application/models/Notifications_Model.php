...
class Notifications_Model extends CI_Model {
	...
	function add_notification($notification) {
		...
		$this->set_default('date', date('Y-m-d H:i:s'));
		$this->set_default('staff_id', $this->session->usr_id);
		$this->db->insert('notifications', $this->notification);
	}

	function set_default($key, $newValue) {
		if(!isset($this->notification[$key]) || $this->notification[$key] <= 0) {
			$this->notification[$key] = $newValue;
		}
	}
}