...
class Products_model extends CI_Model {
	...
	function get_all_products() { 
		$this->db->select('productcategories.name, productcategories.id as categoryid, products.id as id, products.code, products.productname, products.description, products.purchase_price, products.sale_price, products.stock, products.vat, products.status_id, products.productimage, products.product_type, products.unit_id, unit_of_measure.name as unit_measure, products.warehouse_id, warehouses.warehouse_name');
		$this->db->join( 'productcategories', 'products.categoryid = productcategories.id', 'left' );
		$this->db->join( 'unit_of_measure', 'products.unit_id = unit_of_measure.unit_id', 'left' );
		$this->db->join('warehouses', 'warehouses.warehouse_id = products.warehouse_id' ,'left');
	...
	function get_all_products_by_privileges($staff_id='') { 
		$this->db->select('productcategories.name, productcategories.id as categoryid, products.id as id, products.code, products.productname, products.description, products.purchase_price, products.sale_price, products.stock, products.vat, products.status_id, products.productimage, products.product_type, warehouses.warehouse_name');
		$this->db->join( 'productcategories', 'products.categoryid = productcategories.id', 'left' );
		$this->db->join('warehouses', 'warehouses.warehouse_id = products.warehouse_id' ,'left');
		$this->db->order_by( 'products.id', 'desc' );
		if($staff_id) {
	...
	function get_product_by_privileges( $id, $staff_id='' ) {
		$this->db->select('*, productcategories.name, productcategories.id as categoryid, products.id as id, products.code, products.productname, products.description, products.purchase_price, products.sale_price, products.stock, products.vat, products.status_id, products.productimage, products.unit_id as unit_id, unit_of_measure.name as unit_measure, warehouses.warehouse_name, products.warehouse_id');
		$this->db->join( 'productcategories', 'products.categoryid = productcategories.id', 'left' );
		$this->db->join( 'unit_of_measure', 'products.unit_id = unit_of_measure.unit_id', 'left' );
		$this->db->join('warehouses', 'warehouses.warehouse_id = products.warehouse_id' ,'left');
		if($staff_id) {
			return $this->db->get_where( 'products', array( 'products.id' => $id, 'product_created_by' => $staff_id ) )->row_array();
		} else {
			return $this->db->get_where( 'products', array( 'products.id' => $id ) )->row_array();	
		}
	}
	...
	function get_units( $id ) {
		return $this->db->get_where( 'unit_of_measure', array( 'unit_id' => $id ) )->row_array();
	}
/*unit_id not defined*/
	function update_units( $id, $params ) {
		$this->db->where( 'unit_id', $id );
		return $this->db->update( 'unit_of_measure', $params );
	}

	function check_units($id) {
		$data = $this->db->get_where( 'products', array( 'unit_id' => $id ) )->num_rows();
		return $data;
	}

	function remove_units( $id ) {
		$response = $this->db->delete( 'unit_of_measure', array( 'unit_id' => $id ) );
	}

	function isDuplicate( $name ) {
		$data=$this->db->get_where( 'unit_of_measure', array( 'name' => $name ) )->result_array();
		return count($data);
	}
}