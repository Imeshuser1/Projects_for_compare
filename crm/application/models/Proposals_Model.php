...
class Proposals_Model extends CI_Model {
	...
	function proposal_add_by_customer( $params ) {
		...
		$this->Notifications_Model->add_notification([
			'relation_type' =>'proposal',
			'relation' => $proposal,
			'detail' => ( '' . $this->session->name . '' . lang( 'isrequested_quote' ) . '' ),
	...
	function get_all_proposals_filtered_by_privileges($staff_id='',$custID='', $assgID='') {
		$this->db->select( '*,staff.staffname as staffmembername,staff.staffavatar as staffavatar,proposals.id as id' );
		$this->db->join( 'staff', 'proposals.assigned = staff.id', 'left' );
		$this->db->order_by( 'proposals.id', 'desc' );
		if($staff_id) {
			...
			if($custID){
				return $this->db->get_where( 'proposals', array( 'relation' => $custID ) )->result_array();
			}	else{
				return $this->db->get_where( 'proposals', array( 'assigned' => $assgID ) )->result_array();
			}
		}
	}
	...
	function get_proposals_by_privileges( $id, $rel_type, $staff_id='' ) {
		if ( $rel_type == 'customer' ) {
			$this->db->select( '*,staff.staffname as staffmembername,staff.staffavatar as staffavatar,staff.email as staffemail,customers.type as type,customers.company as customercompany,customers.zipcode as zip,customers.email as toemail,customers.namesurname as namesurname,customers.address as toaddress,proposals.status_id as status_id, proposals.id as id, customers.phone as customerphone' );
			$this->db->join( 'customers', 'proposals.relation = customers.id', 'left' );
			$this->db->join( 'staff', 'proposals.assigned = staff.id', 'left' );
		} elseif ( $rel_type == 'lead' ) {
			$this->db->select( '*,staff.staffname as staffmembername,staff.staffavatar as staffavatar,staff.email as staffemail,leads.name as leadname,leads.address as toaddress,leads.email as toemail, proposals.id as id, leads.phone as customerphone ' );
			$this->db->join( 'leads', 'proposals.relation = leads.id', 'left' );
			$this->db->join( 'staff', 'proposals.assigned = staff.id', 'left' );
		}
		if($staff_id){
	...
	function update_sendEmail_date($id, $date){
		// 	Table: 
		return	$response = $this->db->where( 'id', $id )->update( 'proposals', array( 'datesend' => $date) );
	}
}
