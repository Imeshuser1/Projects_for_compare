...
class Projects_Model extends CI_Model {
	...
	function delete_service($id, $number) {
		$this->db->insert( 'logs', array(
			...
			'detail' => ( '' . $this->session->staffname.' '.lang( 'deleted' ).' '.lang('service').' '.lang('for').' '. $number ),
	...
	function delete_tickets( $id ) {
		...
		$this->db->insert( 'logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '' . $this->session->staffname.' '.lang( 'deleted' ).' '.lang('ticket').' '.get_number('tickets',$id,'ticket','ticket') ),
			'staff_id' => $this->session->usr_id
		) );
		return true;
	}
	...
	function add_projects($params)
	{
		...
		$this->db->insert('logs', array(
			'date' => date('Y-m-d H:i:s'),
			'detail' => ('<a href="staff/staffmember/' . $this->session->usr_id . '"> ' . $this->session->staffname . '</a> ' . lang('added') . ' <a href="projects/project/' . $project . '">' . get_number('projects', $project, 'project', 'project')  . '</a>'),
			'staff_id' => $this->session->usr_id
		));
		$this->Notifications_Model->add_notification([
			'relation_type' => 'project',
			'relation' => $project,
			'detail' => ('' . $this->session->staffname . ' ' . lang('created_a') . ' ' . lang('project') . ' ' . get_number('projects', $project, 'project', 'project') . ''),
			'perres' => $this->session->staffavatar,
			'target' => '' . base_url('projects/project/' . $project . '') . ''
		]);
		return $project;
	}
	...
	function update($id, $params)
	{
		...
		$this->db->insert('logs', array(
			'date' => date('Y-m-d H:i:s'),
			'detail' => ('' . $this->session->staffname . ' ' . lang('updated') . ' ' . lang('project') . ' ' . get_number('projects', $id, 'project', 'project')),
			'staff_id' => $loggedinuserid,
			'project_id' => $id,
		));
	}
	...
	function add_milestone( $id, $params ) {
		...
		$this->db->insert( 'logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '' . $this->session->staffname.' '.lang( 'added' ).' '.lang('milestone').' '.lang('for').' '.get_number('projects',$id,'project','project') ),
			'staff_id' => $loggedinuserid,
			'project_id' => $id,
		) );
	...
	function proposal_add( $params ) {
		...
		if(!isset($params['relation'])) {
			$params['relation'] = $params['assigned'];
		}
		$this->Notifications_Model->add_notification([
		...
		$this->db->insert( 'logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '' . $this->session->staffname.' '.lang( 'added' ).' '.lang('proposal').' '.lang('for').' '.get_number('projects',$params['project_id'],'project','project') ),
			'staff_id' => $loggedinuserid,
