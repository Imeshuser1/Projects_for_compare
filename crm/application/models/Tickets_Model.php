...
class Tickets_Model extends CI_Model {
	...
	function get_all_tickets_by_privileges($staff_id='') {
		$this->db->select( '*,departments.name as department,staff.staffname as staffmembername,staff.staffavatar as staffavatar,contacts.name as contactname,contacts.surname as contactsurname, tickets.id as id, contacts.email as contactemail, ticketstatus.name as statusname, customers.company as company,customers.namesurname as namesurname ' );
		$this->db->join( 'ticketstatus', 'tickets.status_id = ticketstatus.id', 'left' );
		$this->db->join( 'contacts', 'tickets.contact_id = contacts.id', 'left' );
		$this->db->join( 'customers', 'tickets.customer_id = customers.id', 'left' );
		$this->db->join( 'departments', 'tickets.department_id = departments.id', 'left' );
		$this->db->join( 'staff', 'tickets.staff_id = staff.id', 'left' );
		$this->db->order_by( 'date desc, priority desc' );
		$this->db->order_by( "date", "desc" );
	...
	function add_status( $params ) {
		$this->db->insert( 'ticketstatus', $params );
		return $this->db->insert_id();
	}

	function update_status( $id, $params ) {
		$this->db->where( 'id', $id );
		return $this->db->update( 'ticketstatus', $params );
	}

	function get_tickets_status() {
		return $this->db->get( 'ticketstatus' )->result_array();
	}

	function check_statuses($id) {
		$data = $this->db->get_where( 'tickets', array( 'status_id' => $id ) )->num_rows();
		return $data;
	}

	function get_status( $id ) {
		return $this->db->get_where( 'ticketstatus', array( 'id' => $id ) )->row_array();
	}

	function delete_status( $id ) {
		$response = $this->db->delete( 'ticketstatus', array( 'id' => $id ) );
	}
}