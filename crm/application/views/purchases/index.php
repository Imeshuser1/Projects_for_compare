...
<div class="ciuis-body-content" ng-controller="Purchases_Controller">
				...
				<md-menu md-position-mode="target-right target">
					...
					<md-menu-content width="4" ng-cloak>
						<md-contet layout-padding>
							<md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.vendor" ng-change="updateColumns('vendor', table_columns.vendor);">
								<?php echo lang('vendor').' '.lang('name') ?>
							</md-checkbox><br>
				...
				<table md-table  md-progress="promise" ng-cloak>
					<thead md-head md-order="purchase_list.order">
						<tr md-row>
							...
							<th ng-show="table_columns.created" md-column md-order-by="created"><span><?php echo lang('billeddate'); ?></span></th>
							<th ng-show="table_columns.duedate" md-column md-order-by="duedate"><span><?php echo lang('invoiceduedate'); ?></span></th>
							...
							<th ng-show="table_columns.staff" md-column md-order-by="staff"><span><?php echo lang('created').' '.lang('by'); ?></span></th>
						</tr>
					</thead>
					<tbody md-body>
						<tr class="select_row" md-row ng-repeat="purchase in purchases | orderBy: purchase_list.order | limitTo: purchase_list.limit : (purchase_list.page -1) * purchase_list.limit | filter: search | filter: FilteredData" class="cursor" ng-click="goToLink('purchases/purchase/'+purchase.id)">
							<td md-cell>
								...
								<small ng-show="table_columns.vendor" ng-bind="purchase.customer"></small>
							</td>
							...
							<td ng-show="table_columns.staff" md-cell>
								<div style="margin-top: 5px;" data-toggle="tooltip" data-placement="left" data-container="body" title="" data-original-title="Created by: {{purchase.staff}}" class="assigned-staff-for-this-lead user-avatar">
									<img ng-src="<?php echo base_url('uploads/images/{{purchase.staffavatar}}')?>" alt="{{purchase.staff}}">   
									<md-tooltip md-direction="left" ng-bind="purchase.staff"></md-tooltip>
								</div>
							</td>
						</tr>
					</tbody>
	...
	<md-content layout-padding="">
		<div ng-repeat="(prop, ignoredValue) in purchases[0]" ng-init="filter[prop]={}" ng-if="prop != 'id' && prop != 'prefix' && prop != 'longid' && prop != 'created' && prop != 'duedate' && prop != 'customer' && prop != 'total' && prop != 'status' && prop != 'color' && prop != 'vendor_id' && prop != 'staff_id' && prop != 'recurring_status' && prop != 'staff' && prop != 'staffavatar' ">
