				...
				<md-menu md-position-mode="target-right target">
					...
					<md-menu-content width="4" ng-cloak>
						<md-contet layout-padding>
							<md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.company" ng-change="updateColumns('company', table_columns.company);">
								<?php echo lang('vendor') ?>
							</md-checkbox><br>
							<md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.group" ng-change="updateColumns('group', table_columns.group);">
								<?php echo lang('group') ?>
							</md-checkbox><br>
							<md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.address" ng-change="updateColumns('address', table_columns.address);">
								<?php echo lang('address') ?>
							</md-checkbox><br>
							<md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.balance" ng-change="updateColumns('balance', table_columns.balance);">
				...
				<table md-table  md-progress="promise" ng-cloak>
					<thead md-head md-order="vendor_list.order">
						<tr md-row>
							...
							<th ng-show="table_columns.company" md-column md-order-by="name"><span><?php echo lang('vendor'); ?></span></th>
							<th ng-show="table_columns.group" md-column md-order-by="group_name"><span><?php echo lang('group'); ?></span></th>
							<th ng-show="table_columns.address" md-column md-order-by="address"><span><?php echo lang('address'); ?></span></th>
							<th ng-show="table_columns.balance" md-column md-order-by="balance"><span><?php echo lang('balance'); ?></span></th>
						</tr>
					</thead>
					<tbody md-body>
						<tr class="select_row" md-row ng-repeat="vendor in vendors | orderBy: vendor_list.order | limitTo: vendor_list.limit : (vendor_list.page -1) * vendor_list.limit | filter: search | filter: FilteredData" class="cursor" ng-click="goToLink('vendors/vendor/'+vendor.id)">
							...
							<td ng-show="table_columns.company" md-cell>
								<strong>
									<strong ng-bind="vendor.name"></strong>
								</strong><br>
								<span class="blur" ng-bind="vendor.email"></span>
							</td>
							<td ng-show="table_columns.group" md-cell>
							...
							<td ng-show="table_columns.balance" md-cell>
								<strong ng-show="vendor.balance !== 0">
				...
				<div class="filter col-md-12">
					...
					<div ng-if="prop=='<?php echo lang('filterbycountry') ?>'">
						<md-select aria-label="Filter" ng-model="filter_select" ng-init="filter_select='all'" ">
							...
							<md-option ng-click="updateDropdown(opt.countryID)" ng-repeat=" opt in byCountry" value="{{opt['countryID']}}">{{opt.countryName}}</md-option>
						</md-select>
					</div>
				</div>
			</div>
