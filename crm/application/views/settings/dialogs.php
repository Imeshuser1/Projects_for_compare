...
<script type="text/ng-template" id="version-check-template.html">
	<md-dialog aria-label="options dialog">
		<?php echo form_open_multipart('settings/download_and_install_update/',array("class"=>"form-horizontal")); ?>
		<md-dialog-content layout-padding>
			...
			<span><?php echo lang('last_updated_on');?>:    
				...
				<p ng-show="msg" class="text-success upto-date-message">{{msg}}</p>
		...
		<md-dialog-actions>
			...
			<md-button ng-if="updated" type="button" ng-click="downloadAndInstallUpdate($event)" class="md-raised md-primary successButton md-button md-ink-ripple" aria-label="Update">
	...
	<md-dialog aria-label="options dialog">
	  ...
	  <md-dialog-content layout-padding class="install-button">
	    ...
	    <div class="text-center">
	      ...
	      <md-button ng-if="install_update" ng-hide="installed == true" type="button" class="md-raised md-primary successButton md-button md-ink-ripple" aria-label="Update" ng-disabled="installing == true">
...
<script type="text/ng-template" id="activate_module.html">
	<md-dialog aria-label="uninstall_license">
		<md-toolbar class="toolbar-white">
			<div class="md-toolbar-tools">
				<h2><strong class="text-success"><?php echo lang('activate_module') ?></strong></h2>
				<span flex></span>
				<md-button class="md-icon-button" ng-click="close()">
					<md-icon class="ion-close-round" aria-label="Close dialog" style="color:black"></md-icon>
				</md-button>
			...
			<div class="">
				<div>
					<md-input-container class="md-block" flex-gt-xs> 
						<label><?php echo lang('enter_license');?></label>
						<input required ng-model="module_license_key">
					</md-input-container>
				</div>
				<div>
					<md-button class="md-raised md-primary" aria-label="Update" ng-disabled="uninstalling == true" ng-click="activateModule(type)">
						<md-progress-circular class="white" md-mode="indeterminate" md-diameter="20" ng-show="checkingModule == true"></md-progress-circular> 
						<span ng-hide="checkingModule == true">
							<?php echo lang('activate_module'); ?>
						</span>
					</md-button>
				</div>
			</div>
...
<script type="text/ng-template" id="deactivate_module.html">
	<md-dialog>
		<md-toolbar class="toolbar-white">
			<div class="md-toolbar-tools">
				<h2><strong class="text-success"><?php echo lang('deactivate_module') ?></strong></h2>
				<span flex></span>
				<md-button class="md-icon-button" ng-click="close()">
					<md-icon class="ion-close-round" aria-label="Close dialog" style="color:black"></md-icon>
				</md-button>
		...
		<md-dialog-content layout-padding class="install-button">
			<div>
				<span class="md-subhead"><?php echo lang('deactivate_module_msg') ?></span>
				<div>
					<md-button class="md-raised md-primary" aria-label="Update" ng-disabled="uninstalling == true" ng-click="deactivateModule(type)">
						<md-progress-circular class="white" md-mode="indeterminate" md-diameter="20" ng-show="checkingModule == true"></md-progress-circular> 
						<span ng-hide="checkingModule == true">
							<?php echo lang('deactivate_module'); ?>
						</span>
					</md-button>
				</div>
			</div>
...
<!-- HR Module -->
<script type="text/ng-template" id="version-check-module-template.html">
	<md-dialog aria-label="options dialog">
		<?php echo form_open_multipart('modules/download_update_module/',array("class"=>"form-horizontal")); ?>
		<md-dialog-content layout-padding>
			<h2 class="md-title" style="border-bottom: 1px solid lightgray;">
				<?php echo lang('version_check'); ?>
			</h2>
			<span><?php echo lang('you_are_usign_version');?> 
			<strong><span>{{Version_detail}}</span></strong></span><a target="_blank" href="https://suisesoft.tech/topics/ciuiscrm/changelog/" class="link">(<?php echo lang('view_changelog');?>)</a> <br>
			<span><?php echo lang('last_updated_on');?>:    
				<strong><span>{{last_updated}}</span></strong></span>
				<p ng-show="updated==''" class="text-success upto-date-message">{{msg}}</p>
				<div ng-if="updated" style="border: 1px dotted #ececec;margin-top: 2%;padding-top: unset;">
					<h2 class="md-title"><?php echo lang('availableversion'); ?> {{ Version_latest }}</h2>
					<p ng-bind-html="changeLog"></p>
					<input type="hidden" name="version_number" value="{{Version_latest}}" ng-model="Version_latest" ng-value="Version_latest">
		...
		<md-dialog-actions>
			<span flex></span>
			<md-button ng-if="!updated" class="template-button" ng-click="checkForModuleUpdates($event)" aria-label="Update">
				<span><?php echo lang('check_for_updates');?></span>
			</md-button>
			<md-button ng-if="updated" type="button" ng-click="downloadModuleUpdate($event, $module)" class="md-raised md-primary successButton md-button md-ink-ripple" aria-label="Update">
				<md-tooltip md-direction="top"><?php echo lang('install_update'); ?> </md-tooltip>
				<md-icon aria-label="download update">
					<i class="mdi mdi-download"></i>
				</md-icon> 
...
<script type="text/ng-template" id="install-update-module.html">
	<md-dialog aria-label="options dialog">
	  <?php echo form_open_multipart('modules/download_update_module/',array("class"=>"form-horizontal")); ?>
	  <md-toolbar class="toolbar-white">
	    <div class="md-toolbar-tools">
	  ...
	  <md-dialog-content layout-padding class="install-button">
	    <div>
	      ...
	      <strong><span>{{version_number}}</span></strong></span><a target="_blank" href="https://stellar.ladesk.com/325898-Change-Log" class="link">(<?php echo lang('view_changelog');?>)</a> <br>
	      <span><?php echo lang('last_updated_on');?>:    
	        <strong><span>{{last_updated}}</span></strong>
	      </span>
	    </div>
	    <div class="text-center">
	      <p class="message-box" ng-show="installed == true">{{installed_message}}</p>
	      <md-button ng-if="install_update" ng-hide="installed == true" type="button" ng-click="installUpdateModule($event, $module)" class="md-raised md-primary successButton md-button md-ink-ripple" aria-label="Update" ng-disabled="installing == true">
	        <md-progress-circular class="white" md-mode="indeterminate" md-diameter="20" ng-show="installing == true"></md-progress-circular> 
	        <span ng-show="installing == false">
	          <md-tooltip md-direction="top"><?php echo lang('install_update'); ?> </md-tooltip>
	          <md-icon aria-label="download update">
