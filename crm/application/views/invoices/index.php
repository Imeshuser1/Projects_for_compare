	...
	<div class="ciuis-body-content" ng-controller="Invoices_Controller">
						...
						<md-menu-content width="4" ng-cloak>
							<md-contet layout-padding>
								<md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.customer" ng-change="updateColumns('customer', table_columns.customer);">
									<?php echo lang('customer').' '.lang('name') ?>
								</md-checkbox><br>
						...
						<tbody md-body>
							<tr class="select_row" md-row ng-repeat="invoice in invoices | orderBy: invoice_list.order | limitTo: invoice_list.limit : (invoice_list.page -1) * invoice_list.limit | filter: invoice_search | filter: FilteredData" class="cursor" ng-click="goToLink('invoices/invoice/'+invoice.id)">
								<td md-cell>
									...
									<small ng-show="table_columns.customer" ng-bind="invoice.customer"></small>
								</td>
								<td ng-show="table_columns.staff" md-cell>
									<strong><span ng-bind="invoice.staff"></span></strong>
								</td>
		...
		<md-sidenav class="md-sidenav-right md-whiteframe-4dp" md-component-id="ContentFilter" ng-cloak style="width: 450px;">
			...
			<md-content layout-padding="">
				<div ng-repeat="(prop, ignoredValue) in invoices[0]" ng-init="filter[prop]={}" ng-if="prop != 'id' && prop != 'prefix' && prop != 'longid' && prop != 'created' && prop != 'duedate' && prop != 'customer' && prop != 'total' && prop != 'status' && prop != 'color' && prop != 'customer_id' && prop != 'staff_id' && prop != 'recurring_status' && prop != 'staff'">
					<div class="filter col-md-12">
						...
						<div ng-if="prop=='<?php echo lang('filterbycustomer') ?>'">
							<md-select aria-label="Filter" ng-model="filter_select" ng-init="filter_select='all'" >
								...
								<md-option ng-repeat="opt in cutomerList" value="{{opt.CustomerID}}" ng-click="updateDropdown(opt.CustomerID)">{{opt.CustomerName}}</md-option>
							</md-select>
						</div>
					</div>
				</div>
