	...
	<md-content class="">
		<md-tabs md-dynamic-height md-border-bottom md-selected="ctrl.selectedIndex">
									...
									<div class="widget-counter-group widget-counter-group-right">
										...
										<div class="pull-right">
											<md-input-container class="md-block">
												<md-select id="datepicker"  ng-model="paymentsExpensesByYear" ng-change="getPaymentsExpensesByYear(paymentsExpensesByYear)" style="min-width: 100px;">
												<md-option ng-repeat="year in years" ng-value="year" ng-selected="year == <?= date('Y') ?>">{{year}}</md-option>
		  ...
		  <md-tab label="<?php echo lang('exportdata') ?>"> 
		  	<md-content class="md-padding bg-white">
			  	<div layout-gt-xs="row">
			          ...
			          <md-select ng-model="exportType" name="exportType" required>
			            ...
			            <md-option value="tasktimer"><?php echo lang('timesheets') ?></md-option>
					...
					<table md-table  md-progress="promise">
						...
						<tbody md-body>
							<tr class="select_row" md-row ng-repeat="expense in expenses  | orderBy: expense_list.order | limitTo: expense_list.limit : (expense_list.page -1) * expense_list.limit | filter: expense_search | filter: FilteredData" class="cursor">
								<td md-cell>
									<a class="link"  href="<?php echo base_url('expenses/receipt/{{expense.id}}') ?>">
					...
					<table md-table  md-progress="promise">
						...
						<tbody md-body>
							<tr class="select_row" md-row ng-repeat="project in projects  | orderBy: project_list.order | limitTo: project_list.limit : (project_list.page -1) * project_list.limit | filter: project_search | filter: FilteredData" class="cursor">
								<td md-cell>
									<a class="link"  href="<?php echo base_url('projects/project/{{project.id}}') ?>">
			...
			<md-toolbar class="toolbar-white"  ng-show="timesheetReport=='false'">
				<div class="md-toolbar-tools">
					<h2 flex md-truncate class="text-bold"><?php echo lang('timesheets'); ?></h2>
					<div class="ciuis-external-search-in-table">
						<input ng-model="timesheet_search" class="search-table-external" id="search" name="search" type="text" placeholder="<?php echo lang('search_by').' '.lang('timesheet')?>"> 
						<md-button class="md-icon-button" aria-label="Search">
							<md-icon><i class="ion-search text-muted"></i></md-icon>
						</md-button>
					</div>
					<md-button ng-click="generatePdf('tasktimer',period)" aria-label="New"  class="md-icon-button">
						<img class="download-export-image" ng-hide="pdfLoader == true" src="<?php echo base_url('assets/img/pdf.png');?>">
						<md-tooltip md-direction="bottom"><?php echo lang('pdf').' '.lang('export') ?></md-tooltip>
						<md-progress-circular ng-show="pdfLoader == true" md-mode="indeterminate"
						md-diameter= '20'></md-progress-circular>
					</md-button>
					<md-button ng-click="exportData('tasktimer',period)" class="md-icon-button"  aria-label="New">
						<img class="download-export-image" ng-hide="csvLoader == true" src="<?php echo base_url('assets/img/csv.png');?>">
						<md-tooltip md-direction="bottom"><?php echo lang('csv').' '.lang('export') ?></md-tooltip>
						<md-progress-circular ng-show="csvLoader == true" md-mode="indeterminate"
						md-diameter= '20'></md-progress-circular>
			...
			<md-content  ng-show="timesheetReport=='false'" class="bg-white"> 
				<md-table-container ng-show="timesheets.length > 0">
					<table md-table  md-progress="promise">
						...
						<tbody md-body>
							<tr class="select_row" md-row ng-repeat="timesheet in timesheets  | orderBy: timesheet_list.order | limitTo: timesheet_list.limit : (timesheet_list.page -1) * timesheet_list.limit | filter: timesheet_search | filter: FilteredData" class="cursor">
								<td md-cell>
							...
							<tr class="select_row" md-row>
								...
								<td md-cell>
									<strong ng-bind="totalTime"></strong>
								</td>
							</tr>
						</tbody>
					</table>
				</md-table-container>
				<md-table-pagination ng-show="timesheets.length > 0" md-limit="timesheet_list.limit" md-limit-options="timesheet_limitOptions" md-page="timesheet_list.page" md-total="{{timesheets.length}}" >
				</md-table-pagination>
				<md-content ng-show="!timesheets.length" class="md-padding no-item-data"><?php echo lang('notdata') ?>
			</md-content>	
		</md-content>
		  </md-tab>
		</md-tabs>
