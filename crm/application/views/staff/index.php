...
<div class="ciuis-body-content" ng-controller="Staffs_Controller">
  <div class="main-content container-fluid col-xs-12 col-md-12 col-lg-9" layout="row" layout-wrap>
    ...
    <div class="md-toolbar-tools">
      ...
      <md-menu ng-show="showList==true" md-position-mode="target-right target">
        ...
        <md-menu-content width="4" ng-cloak>
          <md-contet layout-padding>
            ...
            <md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.type" ng-change="updateColumns('type', table_columns.type);">
              <?php echo lang('type') ?>
            </md-checkbox><br>
            <md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.email" ng-change="updateColumns('email', table_columns.email);">
              <?php echo lang('email') ?>
            </md-checkbox><br>
            ...
            <md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.role" ng-change="updateColumns('role', table_columns.role);">
              <?php echo lang('role') ?>
            </md-checkbox><br>
            <md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.timezone" ng-change="updateColumns('timezone', table_columns.timezone);">
              <?php echo lang('timezone'); ?>
            </md-checkbox><br>
            <md-checkbox md-no-ink aria-label="column select" class="md-primary" ng-model="table_columns.language" ng-change="updateColumns('language', table_columns.language);">
      ...
      <md-card md-theme-watch style="margin-top:0px"> 
        <md-card-title>
          <md-card-title-media style="width:100px;height:100px"> 
            <img ng-src="<?php echo base_url('uploads/images/'); ?>{{member.avatar}}" alt="Avatar" class="staff_img"></md-card-title-media>
        ...
        <table md-table  md-progress="promise">
          <thead md-head md-order="staff_list.order">
            <tr md-row>
              ...
              <th ng-show="table_columns.name" md-column><span><?php echo lang('staff'); ?></span></th>
              <th ng-show="table_columns.type" md-column><span><?php echo lang('type'); ?></span></th>
              <th ng-show="table_columns.email" md-column md-order-by="email"><span><?php echo lang('email'); ?></span></th>
              ...
              <th ng-show="table_columns.role" md-column md-order-by="role"><span><?php echo lang('role'); ?></span></th>
              <th ng-show="table_columns.timezone" md-column md-order-by="timezone"><span><?php echo lang('timezone'); ?></span></th>
              <th ng-show="table_columns.language" md-column md-order-by="language"><span><?php echo lang('language'); ?></span></th>
          ...
          <tbody md-body>
            <tr class="select_row" md-row ng-repeat="member in staff | orderBy: staff_list.order | limitTo: staff_list.limit : (staff_list.page -1) * staff_list.limit | filter: search | filter: FilteredData">
              ...
              <td ng-show="table_columns.role" md-cell>
                <span><span class="badge" ng-bind="member.role"></span></span>
              </td>
              <td ng-show="table_columns.timezone" md-cell>
                <span><span ng-bind="member.timezone"></span></span>
              </td>
              <td ng-show="table_columns.language" md-cell>
                <span><span ng-bind="member.language"></span></span>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <md-content>
      <md-content layout-padding>
        ...
        <md-input-container class="md-block">
          ...
          <input type="text" ng-model="staff.phone" class="form-control" id="title" required ng-pattern="/^[-+() \d]+$/" ng-keyup="showPhoneError($event)">
          <p ng-show="!staff.phone" style="color: red" id="phoneError" hidden><?=lang('phone_required')?></p>
        </md-input-container>
