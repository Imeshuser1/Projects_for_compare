    ...
    <md-content class="bg-white">
      <md-content class="task-detail bg-white" layout-padding ng-cloak>
        ...
        <md-input-container class="md-block">
          ...
          <br>
          <textarea id="email_body" class="tinymce" ng-model="template.message"></textarea>
        </md-input-container>
        <?php if (check_privilege('emails', 'edit')) { ?>
          <md-button ng-click="UpdateTemplate()" class="template-button" ng-disabled="saving == true">
           ...
           <md-button class="template-button" ng-disabled="saving == true" data-toggle="modal" data-target="#preview_modal">
...
<!-- Modal -->
<div class="modal fade" id="preview_modal" tabindex="-1" role="dialog" aria-labelledby="preview_modal_title" aria-hidden="true">
  <div class="modal-dialog mail-template-preview-modal-lg" role="document">
...
<script>
  ...
  var app_logo = "<?= $rebrand['app_logo'] ?>";
  var nav_logo = "<?= $rebrand['nav_logo'] ?>";
  var email_banner = "<?= $rebrand['email_banner'] ?>";
  tinymce.init({ 
    // selector:'textarea',
    theme: 'modern',
    //height: 200,
...
+ ",ul[li]",
  ...
  $('#preview_modal').on('show.bs.modal', function(e) {
    ...
    iframe.className = 'preview_iframe';
    $('#preview_modal_body').html(iframe);
    iframe.contentWindow.document.open();
    iframe.contentWindow.document.write($('#email_body').val()
      .replace(/{site_url}/g, BASE_URL)
      .replace(/{logo}/g, app_logo)
      .replace(/{footer_logo}/g, nav_logo)
      .replace(/{email_banner}/g, email_banner)
    );
    iframe.contentWindow.document.close();
    $('#preview_modal_title').text($('#email_subject').val());
  });
</script>