...
// File Manager real-time permission check using CURL POST request
$_fm_permission = [
    'own' => FALSE,
    'all' => FALSE,
    'create' => FALSE,
...
try {
    $_fm_url = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . explode('?', $_SERVER['REQUEST_URI'], 2)[0];
    $_fm_url = substr($_fm_url, 0, strpos($_fm_url, 'application/third_party/vendor/fm'));
    $_fm_curl_handle = curl_init();
    curl_setopt($_fm_curl_handle, CURLOPT_URL, $_fm_url . 'filemanager/get_permission');
    curl_setopt($_fm_curl_handle, CURLOPT_COOKIE, 'cisession=' . (empty($_COOKIE['cisession']) ? NULL : $_COOKIE['cisession']));
    curl_setopt($_fm_curl_handle, CURLOPT_POST, 1);
    curl_setopt($_fm_curl_handle, CURLOPT_RETURNTRANSFER, true);
    $_fm_curl_output = curl_exec($_fm_curl_handle);
    curl_close($_fm_curl_handle);
    $_fm_curl_output = json_decode($_fm_curl_output, TRUE);
    // Set all the permissions
    foreach ($_fm_permission as $key => $val) {
        if (isset($_fm_curl_output[$key])) {
            $_fm_permission[$key] = $_fm_curl_output[$key];
        }
    }
} catch (Exception $e) {
}
// Check if have view permission
if ($_fm_permission['own'] == FALSE && $_fm_permission['all'] == FALSE) {
    echo "You don't have required permissions.<script>window.top.location.href='$_fm_url';</script>";
    exit;
}
...
$config = array(
    ...
    */
    ...
    |C:\xampp\htdocs\ciuiscrm-dev_v197\application\third_party\vendor\fm
    */
    'upload_dir' => '/application/third_party/vendor/fm/uploads/',
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    //******************
    'delete_files'                          => $_fm_permission['delete'],
    'create_folders'                        => $_fm_permission['create'],
    'delete_folders'                        => $_fm_permission['delete'],
    'upload_files'                          => $_fm_permission['create'],
    'rename_files'                          => $_fm_permission['edit'],
    'rename_folders'                        => $_fm_permission['edit'],
    'duplicate_files'                       => $_fm_permission['create'],
    'extract_files'                         => $_fm_permission['create'],
    'copy_cut_files'                        => ($_fm_permission['create'] &&
                                                $_fm_permission['edit'] &&
                                                $_fm_permission['delete']), // for copy/cut files
    'copy_cut_dirs'                         => ($_fm_permission['create'] &&
                                                $_fm_permission['edit'] &&
                                                $_fm_permission['delete']), // for copy/cut directories
    'chmod_files'                           => $_fm_permission['edit'], // change file permissions
    'chmod_dirs'                            => $_fm_permission['edit'], // change folder permissions
    'preview_text_files'                    => $_fm_permission['all'], // eg.: txt, log etc.
    'edit_text_files'                       => $_fm_permission['edit'], // eg.: txt, log etc.
    'create_text_files'                     => $_fm_permission['create'], // only create files with exts. defined in $config['editable_text_file_exts']
    'download_files'         				=> $_fm_permission['all'], // allow download files or just preview
